...
import { MediaPickerModal } from "@/components/dashboard/entries/media-picker-modal";
import { ReferencePicker } from "@/components/dashboard/entries/reference-picker";
...
export default function NewEntryPage() {
  ...
  useEffect(() => {
    if (collection?.mode === "VISUAL") {
      setBlocks([]);
      setContent({ title: "" }); // Visual mode usually needs a title
    } else if (collection?.fields) {
      const initial: Record<string, unknown> = {};
      collection.fields.forEach((f: Field) => {
  ...
  // Auto-generate slug from title or first field
  useEffect(() => {
    const titleVal = content.title || (collection?.fields?.[0] ? content[collection.fields[0].slug] : "");
    if (!slug && titleVal && typeof titleVal === "string") {
      setSlug(titleVal.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, ""));
    }
  }, [content, slug, collection]);

  const handleSave = async () => {
    if (!slug) return;
    setIsSaving(true);
    try {
      const entryData = collection?.mode === "VISUAL" 
        ? { ...content, blocks } 
        : content;
      ...
      router.push(`/dashboard/collections/${collectionId}/entries`);
    } catch (err) {
      console.error(err);
      setIsSaving(false);
    }
    ...
    return (
      <div key={field.id} className="space-y-3 group/field">
        ...
        {field.type === "media" && (
          <div 
            onClick={() => {
              setActivePickerField(field.slug);
              setIsMediaPickerOpen(true);
            }}
            className={cn(
              "group p-8 border-2 border-dashed border-border rounded-sm flex flex-col items-center justify-center gap-4 text-ink-faint transition-all cursor-pointer",
              value ? "bg-accent/5 border-accent/30" : "bg-paper hover:bg-canvas hover:border-accent"
            )}
          >
             {value ? (
        ...
        {field.type === "reference" && (
          <div 
            ...
            className={cn(
              "flex items-center justify-between px-4 h-11 bg-paper border border-border rounded-sm text-sm text-ink-muted cursor-pointer transition-all hover:border-accent",
              value && "text-ink border-accent/30 bg-accent/5"
            )}
          >
            <span>{value ? `Linked: ${value}` : "Select entry reference..."}</span>
            <LinkIcon className="size-3.5 text-ink-faint" />
          </div>
        )}
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto space-y-12 animate-in fade-in duration-1000 pb-32">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 pb-4 border-b border-border-strong/20">
        <div className="flex items-start gap-6">
          ...
          <div className="space-y-1.5">
             <div className="flex items-center gap-2.5 text-[10px] font-mono font-bold uppercase tracking-[0.2em] text-ink-faint mb-2">
              <Link href="/dashboard/collections" className="hover:text-ink transition-colors no-underline">Collections</Link>
              <ChevronRight className="size-3 opacity-30" />
              <Link href={`/dashboard/collections/${collectionId}/entries`} className="hover:text-ink transition-colors no-underline">{collection?.name || "Collection"}</Link>
              <ChevronRight className="size-3 opacity-30" />
              <span className="text-accent">New Entry</span>
            </div>
            <h1 className="font-display text-4xl font-semibold text-ink">
              Create <em className="italic text-accent not-italic">{collection?.mode === "VISUAL" ? "Page" : "Entry"}</em>
            </h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <Button 
            variant="outline" 
            className="h-11 px-6 text-[11px] font-bold uppercase tracking-widest rounded-sm border-border bg-paper hover:bg-canvas"
          ...
          >
            {isSaving ? (
              ...
              <>
                ...
                {published ? "Publish Entry" : "Save as Draft"}
              </>
            )}
          </Button>
        </div>
            ...
            <CardContent className="p-12 space-y-12 relative z-10">
              <div className="space-y-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="text-2xl font-display font-semibold text-ink">Entry Configuration</h2>
                </div>
                
                <div className="space-y-3">
                  <Label className="block font-mono text-[10px] font-bold text-ink-muted uppercase tracking-[0.2em]">
                    Entry Slug <span className="text-destructive">*</span>
                  </Label>
                  <div className="relative">
                    <LinkIcon className="absolute left-4 top-1/2 -translate-y-1/2 size-4 text-ink-faint" />
                    <Input 
                      value={slug}
                      onChange={(e) => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]+/g, "-"))}
                      className="bg-canvas border-border rounded-sm h-11 pl-11 font-mono text-sm"
                      placeholder="e.g. my-awesome-post"
                    />
                  </div>
                  <p className="text-[10px] text-ink-faint font-light italic">
                    The unique identifier for this entry in the API and preview URLs.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="text-2xl font-display font-semibold text-ink">
                    {collection?.mode === "VISUAL" ? "Visual Canvas" : "Content Data"}
                  </h2>
                </div>
                ...
                <div className="space-y-12">
                  {collection?.mode === "VISUAL" ? (
                    <div className="space-y-10 animate-in fade-in duration-500">
                       <div className="space-y-3">
                        <Label className="block font-mono text-[10px] font-bold text-ink-muted uppercase tracking-[0.2em]">
                          Entry Title <span className="text-destructive">*</span>
                        </Label>
                        <Input 
                          value={(content.title as string) || ""}
                          onChange={(e) => setContent(prev => ({ ...prev, title: e.target.value }))}
                          className="bg-canvas border-border rounded-sm h-12 text-lg font-display font-semibold"
                          placeholder="e.g. My New Landing Page"
                        />
                      </div>
                    ...
                    <>
                      ...
                      {(!collection?.fields || collection.fields.length === 0) && (
                        <div className="py-32 text-center bg-canvas/30 rounded-sm border border-dashed border-border">
                          <p className="text-ink-muted text-sm font-light mb-6">This collection has no fields defined.</p>
          ...
          <div className="p-8 bg-sidebar rounded-sm border-none relative overflow-hidden group">
             ...
             <div className="relative z-10 space-y-6">
               ...
               <div className="space-y-4 font-mono text-[10px] leading-relaxed text-white/40">
                 <div>
                   <p className="text-white/20 uppercase tracking-tighter mb-1">Collection ID</p>
                   <p className="text-white/60 truncate">{collectionId}</p>
                 </div>
                 <div>
                   <p className="text-white/20 uppercase tracking-tighter mb-1">Slug Reference</p>
                   <p className="text-white/60">{collection?.slug || "..."}</p>
                 </div>
                 <div className="pt-2">
      ...
      <MediaPickerModal 
        ...
        onSelect={(url) => {
          if (activePickerField) {
            setContent(prev => ({ ...prev, [activePickerField]: url }));
          }
          setIsMediaPickerOpen(false);
        }}
        selectedUrl={activePickerField ? (content[activePickerField] as string) : undefined}
      />
      ...
      <ReferencePicker 
        ...
        onSelect={(id, title) => {
          if (activePickerField) {
            setContent(prev => ({ ...prev, [activePickerField]: title }));
          }
          setIsReferencePickerOpen(false);
        }}
        selectedId={activePickerField ? (content[activePickerField] as string) : undefined}
      />
    </div>
  );
}
