...
function ReferenceLabel({ id }: { id: string }) {
  const { data: entry, isLoading } = useEntry(id);
  if (isLoading) return <span className="animate-pulse">Loading...</span>;
  if (!entry) return <span className="text-destructive font-mono uppercase text-[10px]">Broken Ref</span>;
  return <span>{entry.data?.title || entry.slug}</span>;
}

export default function EditEntryPage() {
  const { id: collectionId, entryId } = useParams() as { id: string; entryId: string };
  ...
  useEffect(() => {
    if (entry) {
      ...
      setContent(data);
      setSlug(entry.slug);
      if (collection?.mode === "VISUAL" && data.blocks) {
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 pb-4 border-b border-border-strong/20">
        <div className="flex items-start gap-6">
          ...
          <div className="space-y-1.5">
            ...
            <h1 className="font-display text-4xl font-semibold text-ink">
              Edit <em className="italic text-accent not-italic">{collection?.mode === "VISUAL" ? "Page" : "Entry"}</em>
            </h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <SandboxPreview 
            ...
            data={collection?.mode === "VISUAL" ? { ...content, blocks } : content}
          ...
          >
            {isSaving ? (
              ...
              <>
                ...
                {entry?.status === "PUBLISHED" ? "Update Entry" : "Save Changes"}
              </>
            )}
          </Button>
        </div>
            ...
            <CardContent className="p-12 space-y-12 relative z-10">
              <div className="space-y-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="text-2xl font-display font-semibold text-ink">
                    {collection?.mode === "VISUAL" ? "Visual Canvas" : "Entry Configuration"}
                  </h2>
                </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-3 mb-4">
                  <div className="size-1.5 h-6 bg-accent-bright" />
                  <h2 className="text-2xl font-display font-semibold text-ink">
                    {collection?.mode === "VISUAL" ? "Visual Canvas" : "Content Details"}
                  </h2>
                </div>
          ...
          <Card className="bg-paper border-border rounded-sm shadow-sm">
            <CardContent className="p-8 space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between group">
                  <div className="flex items-center gap-3 text-[11px] font-bold uppercase tracking-widest text-ink-muted">
                    <Eye className="size-3.5 opacity-40 group-hover:opacity-100 transition-opacity" />
                    Preview
                  </div>
                  <button 
                    onClick={() => setIsPreviewOpen(true)}
                    className="text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-sm border border-border hover:bg-canvas transition-all"
                  >
                    Launch Sandbox
                  </button>
                </div>
