    ...
    <div className="space-y-4">
      <div className="flex justify-between items-end">
        ...
        <span className="text-[12px] font-semibold text-ink font-mono">
          ...
          <span className="text-ink-muted font-light">/</span> {displayMax}{" "}
          {unit}
        </span>
      </div>
...
export default function DashboardOverview() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      setWelcomeDismissed(
        localStorage.getItem("flowcms_welcome_dismissed") === "true",
      );
      setChecklistDismissed(
        localStorage.getItem("flowcms_checklist_dismissed") === "true",
      );
    }
  }, []);
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 pb-4 border-b border-border-strong/20">
        <div className="space-y-2">
          ...
          <p className="text-ink-muted text-base max-w-lg font-light leading-relaxed">
            Manage your content orchestration and delivery pipeline from a
            single editorial interface.
          </p>
        </div>
        <div className="flex gap-4">
          ...
          <Button
            asChild
            className="h-12 px-8 text-[11px] font-bold uppercase tracking-widest rounded-sm shadow-lg"
          >
            ...
            <Card className="bg-paper border-border rounded-sm p-8 shadow-sm overflow-hidden relative">
              ...
              <CardHeader className="p-0 mb-8">
                ...
                <CardDescription className="font-light text-ink-muted text-xs leading-relaxed mt-1">
                  Your active package allocation ceilings on the{" "}
                  {activePlan.toLowerCase()} tier.
                </CardDescription>
              </CardHeader>
              <div className="space-y-8">
                ...
                <UsageBar
                  ...
                  current={Number(
                    ((stats?.storageBytes ?? 0) / (1024 * 1024 * 1024)).toFixed(
                      2,
                    ),
                  )}
                  max={storageLimitGb}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-ink-muted font-light leading-relaxed">
              Verify database requests, content creation velocities, and growth
              trends.
            </p>
          </div>
